...
class NeptuneHMS(MDApp):
    ...
    def on_start(self):
        try:
            expiry = requests.get(url=f"{self.url_path}validate-expiry/").json()
            if expiry["Message"] == "Expired":
                self.subscription_form()
                return
        except ConnectionError as e:
            Clock.schedule_once(lambda dt: self.show_snack("Network error. Make sure server is on."))
            Clock.schedule_once(lambda dt: self.stop())
        except RequestException:
            Clock.schedule_once(lambda dt: self.show_snack("Couln't fetch response!"))
        try:
            response = requests.get(url=f"{self.url_path}ensure-admin-exist/").json()
            if 'detail' in response:
                self.show_snack(response['detail']) 
            elif 'Message' in response:
                self.show_snack(response['Message'])
            else:
                self.show_snack("An error occured! Please try again.")
                return
    ...
    def validate_worker(self, worker_role):
        ...
        try:
            worker = requests.get(url=f"{self.url_path}signin-worker-data/{worker_name}/{worker_role.lower()}/").json()
        except:
        ...
        if self.user_password.text.strip() != worker["Password"]:
        ...
        if worker_role == "admin":
            ...
            self.signed_in_user_id = worker["ID"]
    ...
    def renew_plan(self):
        ...
        if not activation_key:
            ...
            return
        response = requests.put(url=f"{self.url_path}renew-activation/{activation_key}").json()
        if response["Message"] == "Renewed":
            self.subscription_dialog.dismiss()
            self.show_snack("Your plan renewed! Enjoy :-)")
            return
        self.show_snack(text=response["Message"])
    ...
    def add_patients(self, *args):
        ...
        dob = self.patient_dob.text.strip() if self.patient_dob.text.strip() else "2000-01-01"
        if not "@" in email:
            self.show_snack("Invalid email format!")
            return
        if not phone.isdigit() or not phone.startswith("0"):
            self.show_snack("Invalid phone number format! Should be 07(1)XXXXXXXX")
            return
        if not id_no.isdigit() or not len(id_no) == 8:
            self.show_snack("Invalid ID number!")
            return
        if gender not in ["male", "female"]:
            self.show_snack("Invalid gender")
            return
        if not self.is_valid_date(dob):
            self.show_snack("Invalid date format. Expected YYYY-MM-DD")
            return
        new_patient = {
            "patient_name": name,
            "patient_email": email,
        ...
        try:
            response = requests.post(url=f"{self.url_path}add-patient-data", json=new_patient).json()
            if 'detail' in response:
                self.show_snack(response['detail']) 
            elif 'Message' in response:
                self.show_snack(response['Message'])
                return
            self.show_snack("Patient added successfully!")
            if self.root.current == "admin_screen":
                ...
                prev, prev_cont, prev_header =  self.root.ids.rec_prev, self.root.ids.rvb, self.root.ids.rec_cont_header
            self.show_drugs("Any", "Any", "Any", prev, prev_cont, prev_header)
            self.show_patients("Any", "Any", "Any", prev, prev_cont, prev_header)
        except ValueError:
            self.show_snack("Invalid data sent!!")
    ...
    def appointment_form(self):
        ...
        services = requests.get(url=f"{self.url_path}all-services-data/").json()
        service_btn = MDIconButton(icon = "chevron-down", pos_hint = {"center_y":.5}, on_release = lambda x: self.show_dropdown([service["Name"] for service in services], service_btn, self.fill_services))
    ...
    def add_appointments(self, *args):
        ...
        consultant_name = self.consultant_name.text.strip().lower() if self.consultant_name.text.strip() else "consultant_name"
        service_name = self.service_name.text.strip().lower() if self.service_name.text.strip() else "service_name"
        service_desc = self.service_desc.text.strip().lower() if self.service_desc.text.strip() else "service_desc"
        date_scheduled = self.schedule_date.text.strip() if self.schedule_date.text.strip() else "2000-01-01"
        time_string = self.schedule_time.text.strip() if self.schedule_time.text.strip() else "00:00"
        if not self.is_valid_date(date_scheduled):
            self.show_snack("Invalid date format. Expected YYYY-MM-DD")
            return
        if not self.is_valid_time(time_string):
            self.show_snack("Invalid time format. Expected HH:MM: PM/AM")
            return
        ...
        try:
            response = requests.post(url=f"{self.url_path}add-appointment-data/{patient_name}/{consultant_name}/{service_name}", json=new_appointment).json()
            if 'detail' in response:
                self.show_snack(response['detail'])
                return
            ...
            if self.root.current == "admin_screen":
                ...
                prev, prev_cont, prev_header =  self.root.ids.rec_prev, self.root.ids.rvb, self.root.ids.rec_cont_header
            self.show_appointments("Any", "Any", "Any", prev, prev_cont, prev_header)
        except ValueError:
            self.show_snack("Invalid data sent!!")
    ...
    def show_patients(self, sort_term, sort_dir, search_term, prev, prev_box, pat_header):
        ...
        pats = requests.get(url=f"{self.url_path}/all-patient-data/{sort_term}/{sort_dir}/{search_term}/").json()
        ...
        for pat in pats:
            dob = datetime.strptime(pat["DOB"], "%Y-%m-%d").date()
            added = datetime.strptime(pat["Date"], "%Y-%m-%d").date()
            ...
            data.append({
                ...
                'show_profile': lambda pat_name = pat["Name"], pat_email = pat["Email"], pat_phone = pat["Phone"], pat_id = pat["ID NO"], pat_gender = pat["Gender"], pat_address = pat["Address"]: self.display_patients(pat_name, pat_email, pat_id, pat_phone, pat_gender, pat_address)
            })
    ...
    def show_drugs(self, sort_term, sort_dir, search_term, prev, prev_cont, prev_header):
        ...
        drugs = requests.get(url=f"{self.url_path}all-drug-data/{sort_term}/{sort_dir}/{search_term}/").json()
        for drug in drugs:
            print(drug)
        prev = prev
        ...
        for drug in drugs:
            data.append({
                ...
                'drug_desc': drug["Desc"],
                'drug_quantity': str(drug["Qty"]),
                'drug_price': str(drug["Price"]),
                'drug_expiry': str(drug["Expiry"]),
                'best_before': "Safe" if datetime.strptime(drug["Expiry"], "%Y-%m-%d").date() > datetime.utcnow().date() else "Expired",
                'status': "Sufficient" if int(drug["Qty"]) > 10 else "Restock",
                'remove_drug': lambda x = drug["ID"]: self.remove_drug(x),
                'edit_drug': lambda x = drug["ID"]: self.edit_drugs_form(x),
                'show_profile': lambda drug_name = drug["Name"], drug_cat = drug["Category"], drug_desc = drug["Desc"], drug_qty = drug["Qty"], drug_price = drug["Price"], drug_expiry = drug["Expiry"] : self.display_drugs(drug_name, drug_cat, drug_desc, drug_price, drug_qty, drug_expiry)
            })
    ...
    def show_appointments(self, sort_term, sort_dir, search_term, prev, prev_box, app_header):
        ...
        apps = requests.get(url=f"{self.url_path}all-appointment-data/{sort_term}/{sort_dir}/{search_term}/").json()
        prev = prev
        ...
        for app in apps:
            data.append({
                ...
                'consultant_name': app["Doctor"],
                'service_name': app["Service"],
                'service_desc': app["Desc"],
                'date_scheduled': str(app["Scheduled"]),
                'time_scheduled': str(app["Time"]),
                'date_added': str(app["Requested"]),
                'show_profile': lambda pat_name = app["Patient"], consultant = app["Doctor"], service = app["Service"], objective = app["Desc"], set_on = app["Scheduled"], start_at = app["Time"]: self.display_appointments(pat_name, consultant, service, objective, set_on, start_at)
            })
    ...
    def add_workers(self, *args):
        ...
        if gender not in ["male", "female"]:
            self.show_snack("Invalid gender")
            return
        dob = self.worker_dob.text.strip()
        if not self.is_valid_date(self.worker_dob.text.strip()):
            self.show_snack("Invalid date format. Expected YYYY-MM-DD")
            return
        ...
        try:
            response = requests.post(url=f"{self.url_path}/add-worker-data", json=new_worker).json()
            if 'detail' in response:
                self.show_snack(response['detail']) 
            elif 'Message' in response:
                self.show_snack(response['Message'])
    ...
    def billing_form(self):
        ...
        payment = requests.get(url=f"{self.url_path}all-payments/{pat_name}").json()
        test_fees = sum(t["Fee"] for t in payment["Tests"])
        for t in payment["Tests"]:
            content.add_widget(MDLabel(text=f"Lab Test Fee ({t['Test']})                                Ksh. {t['Fee']}"))

        pharma_fees = sum(d["Fee"] for d in payment["Drugs"])
        for d in payment["Drugs"]:
            content.add_widget(MDLabel(text=f"{d['Drug']} x {d['Count']}                                   Ksh. {d['Fee']}"))

        cons_fees = sum(c["Fee"] for c in payment["Cons"])
        for c in payment["Cons"]:
            content.add_widget(MDLabel(text=f"{c['Service Name']} :                                Ksh. {c['Fee']}"))

        total = test_fees + pharma_fees + cons_fees
        content.add_widget(MDDivider())
        content.add_widget(MDLabel(text=f"Total :                                      Ksh. {total}"))
    ...
    def request_form(self):
        ...
        tests = requests.get(url=f"{self.url_path}all-test-data/").json()
        service_btn = MDIconButton(icon = "chevron-down", pos_hint = {"center_y":.5}, on_release = lambda x: self.show_dropdown([test["Name"] for test in tests], service_btn, self.fill_tests))
    ...
    def add_drugs(self, *args):
        ...
        if not self.is_valid_date(self.drug_expiry.text.strip()):
            self.show_snack("Invalid date format. Expected YYYY-MM-DD")
            return
        expiry = datetime.strptime(self.drug_expiry.text.strip(), "%Y-%m-%d").date() if self.drug_expiry.text.strip() else datetime.strptime("2000-01-01", "%Y-%m-%d").date()
        new_drug = {
            "drug_name": name,
            "drug_category": cat,
            "drug_desc": desc,
            "drug_quantity": qty,
            "drug_price": price,
            "drug_expiry": datetime.strftime(expiry, "%Y-%m-%d")
        }
        try:
            response = requests.post(url=f"{self.url_path}add-drug-data", json=new_drug).json()
            if 'detail' in response:
                self.show_snack(response['detail'])
            elif 'Message' in response:
                self.show_snack(response['Message'])
            self.show_snack(f"Drug added successfully")
            if self.root.current == "admin_screen":
                prev, prev_cont, prev_header =  self.root.ids.adm_prev, self.root.ids.adrvb, self.root.ids.adm_cont_header
            else:
    ...
    def add_prescriptions(self, *args):
        ...
        drugs = requests.get(url=f"{self.url_path}all-drug-data/Search/Any/{drug_name}/").json()
        pats = requests.get(url=f"{self.url_path}/all-patient-data/Search/Any/{pat_name}/").json()
        if not pats or not drugs:
            self.show_snack("Invalid patient or drug name!")
            return
        for drug in drugs:
            if datetime.strptime(drug["Expiry"], "%Y-%m-%d").date() < datetime.utcnow().date():
                return self.show_snack(f"{drug['Name']} is expired and unsafe!!")

            if int(drug["Qty"]) < qty:
                return self.show_snack(f"Insufficient {drug['Name']} available, only {drug['Qty']}!!")
        presc_data = {
            "drug_qty": qty,
        ...
        try:
            response = requests.post(
                url=f"{self.url_path}add-prescription-data/{pat_name}/{drug_name}/{int(self.signed_in_user_id)}",
                json=presc_data
                ).json()
            if 'detail' in response:
                self.show_snack(response['detail']) 
            ...
            if self.root.current == "admin_screen":
                ...
                prev, prev_cont, prev_header =  self.root.ids.diag_prev, self.root.ids.diagrvb, self.root.ids.diag_cont_header           
            self.show_prescriptions("Any", "Any", "Any", prev, prev_cont, prev_header)
        except ValueError:
            self.show_snack("Invalid data sent!!")
    ...
    def add_diagnosis(self, *args):
        ...
        pats = requests.get(url=f"{self.url_path}/all-patient-data/Search/Any/{pat_name}/").json()
        if not pats:
            return self.show_snack("Invalid patient name!!")
        new_diagnosis = {
            "symptoms": symptoms,
            "findings": findings,
        ...
        try:
            response = requests.post(url=f"{self.url_path}add-diagnosis-data/{pat_name}/{int(self.signed_in_user_id)}", json=new_diagnosis).json()
            if 'detail' in response:
                self.show_snack(response['detail']) 
            elif 'Message' in response:
                self.show_snack(response['Message'])
            ...
            if self.root.current == "admin_screen":
                ...
                prev, prev_cont, prev_header =  self.root.ids.diag_prev, self.root.ids.diagrvb, self.root.ids.diag_cont_header           
            self.show_diagnosis("Any", "Any", "Any", prev, prev_cont, prev_header)
        except ValueError:
            self.show_snack("Invalid data sent!!")
    ...
    def add_services(self, *args):
        ...
        service_desc = self.serv_desc.text.strip().lower() if self.serv_desc.text.strip() else "service_desc"
        ...
        try:
            response = requests.post(url=f"{self.url_path}add-services-data/", json=new_service).json()
            if 'detail' in response:
                self.show_snack(response['detail']) 
            elif 'Message' in response:
                self.show_snack(response['Message'])
                return
            self.show_snack("Service added successfully")
    ...
    def add_tests(self, *args):
        ...
        try:
            response = requests.post(url=f"{self.url_path}add-tests-data/", json=new_test).json()
            if 'detail' in response:
                self.show_snack(response['detail']) 
            elif 'Message' in response:
                self.show_snack(response['Message'])
                return
            self.show_snack("Test added successfully!")
    ...
    def add_lab_result(self, *args):
        ...
        conclusion = self.conclusions.text.strip().lower() if self.conclusions.text.strip() else "conclusions"
        ...
        try:
            response = requests.post(url=f"{self.url_path}add-labresult-data/{pat_name}/{int(self.signed_in_user_id)}", json=new_lab_result).json()
            if 'detail' in response:
                self.show_snack(response['detail']) 
            elif 'Message' in response:
                self.show_snack(response['Message'])
                return
            self.show_snack("Result added successfully!")
            if self.root.current == "admin_screen":
                prev, prev_cont, prev_header =  self.root.ids.adm_prev, self.root.ids.adrvb, self.root.ids.adm_cont_header
            else:
                prev, prev_cont, prev_header =  self.root.ids.lab_prev, self.root.ids.labrvb, self.root.ids.lab_cont_header           
            self.show_drugs("Any", "Any", "Any", prev, prev_cont, prev_header)
            self.show_lab_results("Any", "Any", "Any", prev, prev_cont, prev_header)
        except ValueError:
            self.show_snack("Invalid data sent!!")
    ...
    def add_lab_request(self, *args):
        ...
        if not pat_name or not test_name:
            self.show_snack("Please enter patient of test name(s)")
            return
        pats = requests.get(url=f"{self.url_path}/all-patient-data/Search/Any/{pat_name}/").json()
        if not pats:
            self.show_snack("Invalid patient name!")
            return
        try:
            response = requests.post(url=f"{self.url_path}add-labrequest-data/{pat_name}/{int(self.signed_in_user_id)}/{test_name}").json()
            if 'detail' in response:
                self.show_snack(response['detail']) 
            elif 'Message' in response:
                self.show_snack(response['Message'])
                return
            self.show_snack("Request sent successfully!")
            if self.root.current == "admin_screen":
                prev, prev_cont, prev_header =  self.root.ids.adm_prev, self.root.ids.adrvb, self.root.ids.adm_cont_header
            else:
                prev, prev_cont, prev_header =  self.root.ids.diag_prev, self.root.ids.diagrvb, self.root.ids.diag_cont_header           
            self.show_drugs("Any", "Any", "Any", prev, prev_cont, prev_header)
            self.show_lab_requests("Any", "Any", "Any", prev, prev_cont, prev_header)
        except ValueError:
            self.show_snack("Invalid data sent!!")
    ...
    def show_workers(self, sort_term, sort_dir, search_term):
        ...
        wrks = requests.get(url=f"{self.url_path}all-worker-data/{sort_term}/{sort_dir}/{search_term}/").json()
        ...
        for wrk in wrks:
            dob = datetime.strptime(wrk["DOB"], "%Y-%m-%d").date()
            added = datetime.strptime(wrk["Date"], "%Y-%m-%d").date()

            age = added.year - dob.year - ((added.month, added.day) < (dob.month, dob.day))

            data.append({
                'worker_name': wrk["Name"],
                'worker_email': wrk["Email"],
                'worker_phone': wrk["Phone"],
                'worker_id_no': wrk["ID NO"],
                'worker_gender': wrk["Gender"],
                'worker_role': wrk["Role"],
                'worker_dob': dob.strftime("%Y-%m-%d"),
                'worker_age': str(age),
                'worker_date_added': added.strftime("%Y-%m-%d"),
                'remove_worker': lambda x = wrk["ID"]: self.remove_worker(x),
                'show_profile': lambda wrk_name = wrk["Name"], wrk_email = wrk["Email"], wrk_phone = wrk["Phone"], wrk_id = wrk["ID NO"], wrk_gender = wrk["Gender"], wrk_role = wrk["Role"]: self.display_workers(wrk_name, wrk_email, wrk_id, wrk_phone, wrk_gender, wrk_role)
            })
    ...
    def show_prescriptions(self, sort_term, sort_dir, search_term, prev, prev_cont, prev_header):
        ...
        prescs = requests.get(url=f"{self.url_path}all-prescription-data/{sort_term}/{sort_dir}/{search_term}/").json()
        prev = prev
        ...
        for presc in prescs:
            data.append({
                'patient_name': presc["Patient"],
                'prescription': presc['Presc'],
                'drug_name': presc['Drugs'],
                'drug_quantity': presc['Qty'],
                'pharmacist': presc['Prescriber'],
                'date_added': presc['Date'],
                'show_profile': lambda pat_name = presc["Patient"], drugs = presc["Drugs"], prescription = presc["Presc"], quantity = presc["Qty"], prescriber = presc["Prescriber"] : self.display_prescriptions(pat_name, prescription, drugs, quantity, prescriber)
            })
        print(data)   
        prev.data = data
    ...
    def show_diagnosis(self, sort_term, sort_dir, search_term, prev, prev_cont, prev_header):
        ...
        prev = prev
        diags = requests.get(url=f"{self.url_path}all-diagnosis-data/{sort_term}/{sort_dir}/{search_term}/").json()
        header = prev_header
        header.clear_widgets()
        ...
        for diag in diags:
            ...
            data.append({
                'patient_name': diag["Patient"],
                'symptoms': diag["Symptoms"],
                'findings': diag["Findings"],
                'suggested_diagnosis': diag["Diagnosis"],
                'diagnoser': diag["Doctor"],
                'date_added': str(diag["Date"]).split(" ")[0],
                'show_profile': lambda pat_name = diag["Patient"], symptoms = diag["Symptoms"], findings = diag["Findings"], diagnosis = diag["Diagnosis"], diagnoser = diag["Doctor"]: self.display_diagnoses(pat_name, symptoms, findings, diagnosis, diagnoser)
            })
        
        prev.data = data
    
    def show_billings(self, sort_term, sort_dir, search_term, prev, prev_cont, prev_header):
        bills = requests.get(url=f"{self.url_path}all-billing-data/{sort_term}/{sort_dir}/{search_term}/").json()
        prev = prev
    ...
    def show_services(self, prev, prev_cont, prev_header):
        ...
        servs = requests.get(url=f"{self.url_path}all-services-data/").json()
        ...
        for serv in servs:
            ...
            data.append({
                ...
                'service_price': str(serv["Price"]),
                'service_desc': serv["Desc"],
            })
    ...
    def show_lab_results(self, sort_term, sort_dir, search_term, prev, prev_cont, prev_header):
        ...
        resuls = requests.get(url=f"{self.url_path}all-labresult-data/{sort_term}/{sort_dir}/{search_term}/").json()
        prev = prev
        ...
        for resul in resuls:
            ...
            data.append({
                'patient_name': resul["Patient"],
                'observations': resul["Observations"],
                'conclusions': resul["Conclusion"],
                'labtech': resul["Tech"],
                'date_added': str(resul["Date"]).split(" ")[0],
                'show_profile': lambda pat_name = resul["Patient"], obs = resul["Observations"], conc = resul["Conclusion"], labtech = resul["Tech"]: self.display_resuls(pat_name, obs, conc, labtech)
            })
    ...
    def show_lab_requests(self, sort_term, sort_dir, search_term, prev, prev_cont, prev_header):
        ...
        reqs = requests.get(url=f"{self.url_path}all-labrequest-data/{sort_term}/{sort_dir}/{search_term}/").json()
        prev = prev
        ...
        for request in reqs:
            ...
            data.append({
                ...
                'date_added': str(request["Date"]).split(" ")[0]
            })
    ...
    def on_select_day(self, instance, value):
        self.day = f"{int(value):02}" if value else "00"

    def on_select_month(self, instance, value):
        self.month = f"{int(value):02}" if value else "00"
    ...
    def remove_worker(self, worker_id):
        response = requests.delete(url=f"{self.url_path}delete-worker-data/{worker_id}/").json()
        self.show_snack(f"{response["Message"]}")
        self.show_workers("Any", "Any", "Any")
    
    def remove_drug(self, drug_id):
        response = requests.delete(url=f"{self.url_path}delete-drug-data/{drug_id}/").json()
        if self.root.current == "admin_screen":
            prev, prev_cont, prev_header = self.root.ids.adm_prev, self.root.ids.adrvb, self.root.ids.adm_cont_header
...
# Applying searching to various records by conditions
    ...
    def edit_drugs_form(self, drug_id):
        drug = requests.get(url=f"{self.url_path}all-drug-data-by-id/{drug_id}").json()
        self.d_name = MDTextField(MDTextFieldHintText(text = "Drug Name"), text = drug["Name"])
        self.d_category = MDTextField(MDTextFieldHintText(text = "Category"), text = drug["Category"])
        self.d_desc = MDTextField(MDTextFieldHintText(text = "Description"), text = drug["Desc"])
        self.d_price = MDTextField(MDTextFieldHintText(text = "Price"), text = str(drug["Price"]), input_filter = "float")
        self.d_qty = MDTextField(MDTextFieldHintText(text = "Quantity"), text = str(drug["Qty"]), input_filter = "int")
        self.d_expiry = MDTextField(MDTextFieldHintText(text = "Expiry Date"), text = str(drug["Expiry"]).split(" ")[0])
        ...
        self.drug_edit_dialog = MDDialog(
            ...
            MDDialogButtonContainer(
                Widget(),
                MDButton(MDButtonText(text = "Edit"), on_release = lambda x: self.edit_drugs(drug_id=int(drug["ID"]))),
    ...
    def edit_drugs(self, drug_id):
        ...
        new_data = {
            ...
            "drug_expiry": drug_expiry
        }
        response = requests.put(url=f"{self.url_path}update-drug-data/{drug_id}", json=new_data).json()
        self.show_snack(f"{response['Message']}")
    ...
    def export_pat_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            patients = requests.get(url=f"{self.url_path}/all-patient-data/Any/Any/Any/").json()
            writer = csv.writer(file)
            writer.writerow(["Patient", "Email", "Phone", "ID NO", "Gender", "Address", "D.O.B", "Date Added"])
            for p in patients:
                writer.writerow([p["Name"], p["Email"], p["Phone"], p["ID NO"], p["Gender"], p["Address"], p["DOB"], p["Date"]])
    ...
    def export_wrk_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            workers = requests.get(url=f"{self.url_path}all-worker-data/Any/Any/Any/").json()
            writer = csv.writer(file)
            writer.writerow(["Worker", "Email", "Phone", "ID NO", "Gender", "Role", "D.O.B", "Date Added"])
            for w in workers:
                writer.writerow([w["Name"], w["Email"], w["Phone"], w["ID"], w["Gender"], w["Role"], w["DOB"], w["Date"]])
    ...
    def export_app_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            appointments = requests.get(url=f"{self.url_path}all-appointment-data/Any/Any/Any/").json()
            writer = csv.writer(file)
            writer.writerow(["Patient", "Consultant", "Service", "Objective", "Set On", "Start At", "Date Scheduled"])
            for a in appointments:
                writer.writerow([a["Patient"], a["Doctor"], a["Service"], a["Desc"], a["Scheduled"], a["Time"], a["Requested"]])
    ...
    def export_drug_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            drugs = requests.get(url=f"{self.url_path}all-drug-data/Any/Any/Any/").json()
            writer = csv.writer(file)
            writer.writerow(["Drug", "Category", "Description", "Quantity", "Price", "Expiry Date"])
            for d in drugs:
                writer.writerow([d["Name"], d["Category"], d["Desc"], d["Qty"], d["Price"], d["Expiry"]])
    ...
    def export_presc_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            prescriptions = requests.get(url=f"{self.url_path}all-prescription-data/Any/Any/Any/").json()
            grouped = defaultdict(list)
            writer = csv.writer(file)
            writer.writerow(["Patient", "Prescription", "Drugs", "Quantity", "Prescribed By", "Date Prescribed"])

            for presc in prescriptions:
                writer.writerow([presc["Patient"], presc["Presc"], presc["Drugs"], presc["Qty"], presc["Prescriber"], presc["Date"]])
    ...
    def export_lab_result_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            results = requests.get(url=f"{self.url_path}all-labresult-data/Any/Any/Any/").json()
            writer = csv.writer(file)
            writer.writerow(["Patient", "Observations", "Conclusions", "Lab Technician", "Date Conveyed"])
            for r in results:
                writer.writerow([r["Patient"], r["Observations"], r["Conclusion"], r["Tech"], r["Date"]])
    ...
    def export_lab_requests_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            reqs = requests.get(url=f"{self.url_path}all-labrequest-data/Any/Any/Any/").json()
            writer = csv.writer(file)
            writer.writerow(["Patient", "Doctor", "Test Name", "Date Requested"])
            for r in reqs:
                writer.writerow([r["Patient"], r["Doctor"], r["Test"], r["Date"]])
    ...
    def export_diag_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            diags = requests.get(url=f"{self.url_path}all-diagnosis-data/Any/Any/Any/").json()
            writer = csv.writer(file)
            writer.writerow(["Patient", "Symptoms", "Findings", "Diagnoses", "Doctor", "Date Diagnosed"])
            for d in diags:
                writer.writerow([d["Patient"], d["Symptoms"], d["Findings"], d["Diagnosis"], d["Doctor"], d["Date"]])
    ...
    def export_lab_test_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            ...
            for t in tests:
                writer.writerow([t["Name"], t["Desc"], t["Price"]])
    ...
    def export_services_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            ...
            for s in services:
                writer.writerow([s["Name"], s["Desc"], s["Price"]])
    ...
    def export_billings_data(self):
        ...
        with open(full_path, mode="w", encoding="utf-8", newline="") as file:
            bills = requests.get(url=f"{self.url_path}all-billing-data/Any/Any/Any/").json()
            writer = csv.writer(file)
            writer.writerow(["Patient", "Items", "Total", "Date Billed"])
            for b in bills:
                writer.writerow([b["Patient"], b["Items"], b["Total"], b["Date"]])
